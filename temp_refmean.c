...
static int32_t SRDHM(int32_t a, int32_t b) {
    ...
    long long nudge = ab >= 0 ? (1ll << 30) : (1ll + 1 - (1ll << 30));
...
static int32_t MBQM(int32_t x, int32_t qm, int shift) {
...
int main() {
  ...
  for(int c=0;c<384;c++){
    ...
    int32_t out = clip_int8(MBQM(shifted_sum, output_multiplier, output_shift) + output_zp);
    if(out != gold[c]) mism++;
  }
  printf("mism=%d mult=%d shift=%d\n", mism, output_multiplier, output_shift);
  return 0;
}
