...
int main() {
    ...
    // `calloc` đã khởi tạo toàn bộ vùng nhớ là 0.
    // Vì IFM đầu vào đã được trừ đi ZP từ lớp trước, giá trị padding (đại diện cho ZP) cũng phải là 0.
    // Do đó, `calloc` đã chuẩn bị đúng buffer đệm cho chúng ta.
    printf("Padded buffer initialized with 0s. IFM data is pre-subtracted with ZP from previous layer.\n");

    // Chỉ cần chép phần dữ liệu IFM (đã được trừ ZP từ lớp trước) vào vùng trung tâm.
