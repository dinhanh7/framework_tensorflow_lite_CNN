...
// ============================================================================
// 1. ĐỊNH NGHĨA CÁC KÍCH THƯỚC (QUAN TRỌNG: KIỂM TRA LẠI CHO LAYER 5)
// ============================================================================
...
int main() {
    ...
    int32_t* bias = (int32_t*)malloc(OFM_CHANNEL * sizeof(int32_t));
    ...
    // --- Đường dẫn file (Đã cập nhật cho layer 5) ---
    const char* params_path = "c:\\Code c\\Tensorflow\\framework_tensorflow_lite_CNN\\extracted_params\\layer005_CONV_2D_1_stem_conv_1_BiasAdd_1_stem_conv_1_convolution_\\";
    const char* io_path = "c:\\Code c\\Tensorflow\\framework_tensorflow_lite_CNN\\all_layer_io\\layer_5_CONV_2D\\";
    
    char ifm_file[256], weight_file[256], bias_file[256], m_file[256], n_file[256], ifm_zp_file[256], ofm_zp_file[256], ofm_file[256];

    sprintf(ifm_file, "%sifm5.txt", io_path);
    sprintf(weight_file, "%sweight_values.txt", params_path);
    sprintf(bias_file, "%sbias_value.txt", params_path); // Sử dụng bias thường vì đã trừ ZP
    sprintf(m_file, "%sop005_m_values.txt", params_path); // File M mới tạo
    sprintf(n_file, "%sop005_n_values.txt", params_path); // File N mới tạo
    sprintf(ifm_zp_file, "%sifm_zp.txt", params_path);
    sprintf(ofm_zp_file, "%sofm_zp.txt", params_path);
    sprintf(ofm_file, "%sofm5_c_sim.txt", io_path); // Tên file output mới
    ...
    read_int32_array_from_file(bias_file, bias, OFM_CHANNEL);
    ...
    // `calloc` đã khởi tạo toàn bộ vùng nhớ là 0. Vì tích chập thực hiện trên (I - Zi),
    // giá trị padding phải là (Zi - Zi) = 0, nên chúng ta không cần gán lại giá trị padding.
    printf("Padded buffer initialized with 0s for (I - Zi) operation.\n");
    ...
    for (int c = 0; c < IFM_CHANNEL; ++c) {
        for (int h = 0; h < IFM_HEIGHT; ++h) {
            for (int w = 0; w < IFM_WIDTH; ++w) {
                ...
                padded_ifm[padded_idx] = ifm[orig_idx] ; // không trừ zp nữa vì ở lớp trước đã được trừ rồi
            }
        }
    }
    ...
    // --- Xuất accumulator (int32) ra file để debug ---
    {
        ...
        sprintf(acc_debug_file, "%saccumulator_layer5.txt", io_path);
        ...
        if (fp_acc) {
            ...
            for (int i = 0; i < total; ++i)
                fprintf(fp_acc, "%d\n", accumulator[i]);
            fclose(fp_acc);
            printf("Debug: accumulator (int32 decimal) written to %s.\n", acc_debug_file);
        }
    }

    // --- CỘNG BIAS (tách riêng khỏi phần conv) ---
    printf("Adding bias...\n");
    for (int oh = 0; oh < OFM_HEIGHT; ++oh) {
        for (int ow = 0; ow < OFM_WIDTH; ++ow) {
            for (int oc = 0; oc < OFM_CHANNEL; ++oc) {
                int acc_idx = oh * OFM_WIDTH * OFM_CHANNEL + ow * OFM_CHANNEL + oc;
                accumulator[acc_idx] += bias[oc];
            }
        }
    }
